...
```powershell
& '.\tools\Package-Release.ps1' -Version '7.13'
```
...
2026-09-01 Version 7.13 기준 Windows SDK `10.0.26100.0`, `/W4`, `/utf-8`로 네 구성 모두 오류 0개, 자체 코드 경고 0개다. 구성별 952개·총 3,808개 자동 검증과 5종 몬스터, 9개 적 안정 ID, Refresh Peg 최소 보장·매 턴 재배치·파괴 후 즉시 재생성, 오브·유물 보상 안내, 이동 페그, 시작 전 경로 재선택, Goblin Market 상점, 몬스터별 칸 거리·공격 사거리, 이미지 기반 스테이지 카드 클릭 영역을 검증했다. 자산 파이프라인은 22개 리소스와 21개 PNG→24-bit BMP 변환을 검증한다.
