...
```powershell
& '.\tools\Package-Release.ps1' -Version '7.6'
```
...
2026-09-01 Version 7.6 기준 Windows SDK `10.0.26100.0`, `/W4`, `/utf-8`로 네 구성 모두 오류 0개, 자체 코드 경고 0개다. 구성별 774개·총 3,096개 자동 검증과 5종 몬스터, 9개 적 안정 ID, Refresh Peg 직접 재생성·턴 종료 최소 보장, 간결한 스테이지 카드와 연출 완료 후 단일 화면 전환을 통과했다. 자산 파이프라인은 10개 리소스와 9개 PNG→24-bit BMP 변환을 검증한다.
